...
public class GameSessionController {
    ...
    public ResponseEntity<CreateGameResponse> postCreateGame(HttpServletResponse response) {
        ...
        Cookie cookie = new Cookie(GameCookies.GAME_SESSION_ID, gid);
    ...
    public ResponseEntity<JoinGameResponse> postJoinGame(
            ...
            @CookieValue(value = GameCookies.PUBLIC_ID, required = false) String publicId,
            @CookieValue(value = GameCookies.PRIVATE_TOKEN, required = false) String privateToken,
            HttpServletResponse httpServletResponse) {

        JoinDto dto = (publicId == null || privateToken == null)
                ? gameSessionService.join(gid)
                : gameSessionService.join(gid, publicId, privateToken);
        ...
        switch (dto.result()) {
            case JoinResult.SUCCESS -> {
                ...
                Cookie prvCookie = new Cookie(GameCookies.PRIVATE_TOKEN, dto.privateToken());
                ...
                Cookie pubCookie = new Cookie(GameCookies.PUBLIC_ID, dto.participantId());
